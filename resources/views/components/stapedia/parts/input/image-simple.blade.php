...
<div class="{{ isset($option['isLast']) && trim($option['isLast']) === 'on' ? 'mb-4' : 'mb-10' }}">
    ...
    <div class="w-full">
        <label for="{{ $option['name'] }}" class="relative block cursor-pointer image-upload-label"
               data-form-id="{{ $option['name'] }}">
            <div
                class="flex items-center justify-between w-full px-2 py-3 bg-white border border-gray-200 rounded-md hover:bg-gray-50">
            ...
            <input id="{{ $option['name'] }}" name="{{ $option['name'] }}" type="file" class="sr-only image-upload"
                   accept="image/*">
        </label>
