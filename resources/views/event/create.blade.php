            ...
            <div class="w-full sm:w-3/4 mx-auto py-8 px-4">
                ...
                <form class="space-y-12">
                    <div class="bg-white py-10 px-2 sm:px-8">
                        ...
                        @php
                            $imageUploadForms = [
                                ['id' => 'image1', 'label' => '画像①', 'ratio' => '1:1', 'mockupImage' => 'iphone_1.png'],
                                ['id' => 'image2', 'label' => '画像②', 'ratio' => '2:3', 'mockupImage' => 'iphone_2.png'],
                            ];
                        @endphp

                        @foreach ($imageUploadForms as $form)
                            <div class="mb-28">
                                <div class="flex items-center gap-2 mb-2">
                                    <label class="block text-sm font-medium text-gray-700">{{ $form['label'] }}</label>
                                    <span class="inline-flex items-center bg-green-500 px-2 py-1 text-xs font-medium text-white ml-2">必須</span>
                                </div>
                                <p class="text-sm text-gray-600 mb-3">※「幅：横={{ $form['ratio'] }}」にて画像をアップロードしてください。</p>

                                <div class="flex justify-start items-start space-x-20 space-y-4 flex-wrap">
                                    <!-- アップロードエリア -->
                                    <div class="sm:w-1/3 w-full">
                                        <label for="{{ $form['id'] }}" class="relative block cursor-pointer image-upload-label" data-form-id="{{ $form['id'] }}">
                                            <div class="flex items-center justify-between w-full px-4 py-3 bg-white border border-gray-200 rounded-md hover:bg-gray-50">
                                                <span class="text-sm text-gray-600 file-name">画像をアップロードしてください</span>
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                                </svg>
                                            </div>
                                            <input id="{{ $form['id'] }}" name="{{ $form['id'] }}" type="file" class="sr-only image-upload" accept="image/*">
                                        </label>
                                    ...
                                    <div class="block w-[240px] flex-shrink-0">
                                        <img src="{{ url('storage/' . $form['mockupImage']) }}" alt="スマートフォン表示イメージ" class="w-full">
                                    </div>
                                </div>
                            </div>
                        @endforeach
